...
public class QuanLiDiem {
	...
	private void nhapDiem() {
		...
		System.out.println(
				"Nhập điểm cho sinh viên: " + QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getHoDem() + " "
						+ QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getTen() + " Môn học: "
						+ QuanLiMonHoc.list.get(Helper.timMonHocTrungMa(mamh)).getTenMon());
		double diem = 0;
		try {
	...
	private void suaDiem() {
		...
		System.out.println(
				"Nhập điểm sửa cho sinh viên: " + QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getHoDem()
						+ " " + QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getTen() + " Môn học: "
						+ QuanLiMonHoc.list.get(Helper.timMonHocTrungMa(mamh)).getTenMon());
		double diem = 0;
		try {
	...
	private void xoaDiem() {
		...
		System.out.println("Đã xóa sinh viên: " + QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getHoDem()
				+ " " + QuanLiSinhVien.list.get(Helper.timSinhVienTrungMa(masv)).getTen() + " Và môn học: "
				+ QuanLiMonHoc.list.get(Helper.timMonHocTrungMa(mamh)).getTenMon());
	...
	private void hienThiBangDiemSV() {
		...
		for (int i = 0; i < QuanLiSinhVien.list.size(); i++) {
			...
			if (index != -1) {
				...
				System.out.println("Họ tên sinh viên:" + QuanLiSinhVien.list.get(i).getHoDem() + " "
						+ QuanLiSinhVien.list.get(i).getTen());
				double tong = 0;
				double tonghs = 0;
				...
				for (int j = 0; j < list.size(); ++j) {
					...
					if (list.get(j).getMaSV() == QuanLiSinhVien.list.get(i).getMaSV()) { // hiển thi mã môn tên môn hệ
																							// số của môn học
	...
	public void timKiemThongTinSinhVien() {
		...
		System.out.println("--------Bảng điểm của sinh viên " + QuanLiSinhVien.list.get(index).getHoDem() + " "
				+ QuanLiSinhVien.list.get(index).getTen() + "---------");
		System.out.println("Mã sinh viên:" + QuanLiSinhVien.list.get(index).getMaSV());
		System.out.println("Họ tên sinh viên:" + QuanLiSinhVien.list.get(index).getHoDem() + " "
				+ QuanLiSinhVien.list.get(index).getTen());
		double tong = 0;
		double tonghs = 0;
		DecimalFormat de = new DecimalFormat("##.0#");
		...
		for (int j = 0; j < list.size(); ++j) {
			...
			if (list.get(j).getMaSV() == QuanLiSinhVien.list.get(index).getMaSV()) { // hiển thi mã môn tên môn hệ số
																						// của môn học
				MonHoc monHoc = new MonHoc();
				monHoc = Helper.timMonHoc(list.get(j).getMaMon());
				System.out.printf("%-6d ", monHoc.getMaMon());
	...
	public void timKiemThongTinMonHoc() {
		...
		System.out.println(
				"------------Hiển thị bảng điểm môn học " + QuanLiMonHoc.list.get(index1).getTenMon() + "-----------");
		double diemtk = 0, tong = 0, dem = 0;
